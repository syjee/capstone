...
{ 
    ...
    public PeerNetwork(Blockchain bc,KeyManager km,PeerNetworkManager pm,TimeManagement tm,RPCManager rm) 
    { 
        this.listenPort = 8015; 
        this.peerThreads = new ArrayList<PeerThread>();  
        this.newPeers = new ArrayList<String>();
    ...
    { 
        ...
        {   
        	if(!isSelf(peer)) {
            	...
            	peerThreads.add(new PeerThread(socket,bc,km,pm,tm,rm)); 
...
//            	cSocket.add(listenSocket.accept());
 //           	if(!isSelf(cSocket.get(cSocket.size()-1).getInetAddress())) {
            		peerThreads.add(new PeerThread(listenSocket.accept(),bc,km,pm,tm,rm)); 
            		peerThreads.get(peerThreads.size() - 1).start();
...
//            System.out.println("Sent Block:: " + newBlock.getCurrentHash()); 
    ...
    public void unicast(String host,Block newBlock){
        ...
        { 
//            System.out.println("Sent Block:: " + newBlock.getCurrentHash()); 
            peerThreads.get(i).unicast(host,newBlock); 
        }
    }
