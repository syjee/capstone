		...
		while (true) {
			...
			try {
				...
				while ((newBlock = (Block) ois.readObject()) != null) {
					...
					sw.initForTransaction();
					
					String transaction = km.decrypt(newBlock.getTransaction().getTransaction(), 
							km.getPublicKey(socket
							.getInetAddress().toString()));
				
					Transaction t = new Transaction(3, km,transaction,sw);
					newBlock.addTransaction(t);
					pm.unicast("192.168.11.12", newBlock);
