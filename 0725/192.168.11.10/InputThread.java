...
public class InputThread extends Thread {
	...
	public InputThread(Socket socket, Blockchain bc,PeerNetworkManager pm,KeyManager km,TimeManagement tm) {
		this.socket = socket;
		this.bc = bc;
		this.db = new DBHelper();
		this.pm = pm;
		...
		while (true) {
			...
			try {
				...
				while ((newBlock = (Block) ois.readObject()) != null) {
                    ...
                    { 
                        System.out.println("[Peer]Get Transaction in Block:" + newBlock.getTransaction().getTransaction().substring(0, 20) + " from " + socket.getInetAddress()+" : "+socket.getPort());
                        
                    } 
                    else 
                    { 
                        System.out.println("[Peer]Get Transaction in Block:" + newBlock.getTransaction().getTransaction() + " from " + socket.getInetAddress()+" : "+socket.getPort()); 
                    } 
