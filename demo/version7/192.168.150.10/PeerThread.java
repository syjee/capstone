...
public class PeerThread extends Thread {
	...
	public PeerThread(Socket socket, Blockchain bc, KeyManager km,PeerNetworkManager pm,TimeManagement tm) {
		this.socket = socket;
		this.bc = bc;
		this.db = new DBHelper();
		this.km = km;
	...
	public void run() {
		...
		inputThread = new InputThread(socket,bc,pm,km,tm);
		inputThread.start();
	...
	public void unicast(String host, Block newBlock) {
		if (outputThread == null) {
			...
			if (host.equals(remoteHost))
				outputThread.write(newBlock);
			else System.out.println("[Unicast]Couldn't send Block for the host");
		}
	}
