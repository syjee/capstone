...
public class StopWatch{
	...
	public void secToHHMMSS(int secs) {
		...
		ts.timeSync(Long.parseLong(time),hour,min,sec);
		
	}
	
	public void stopwatch2(int onOff) {
		if (onOff == 1){
			System.out.println("[TIME]Timer ON!!!");
			oldTime = (int) System.currentTimeMillis() / 1000;
		}
		...
		if (onOff == 0) // 타이머 끄고, 시분초를 timerBuffer 에 저장
			secToHHMMSS2(((int) System.currentTimeMillis() / 1000) - oldTime);
