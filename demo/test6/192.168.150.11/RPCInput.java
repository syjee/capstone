...
public class RPCInput extends Thread {
	...
	public RPCInput(Socket socket, PeerNetworkManager pm, Blockchain bc,
			KeyManager km,BlockNetworkManager bnm) {
		this.socket = socket;
		this.pm = pm;
	...
	public void run() {
		try {
			 ...
			 * System.out.println("[RPC]Get Transaction: " +
			 * newTransaction.getTransaction()); bg = new
			 * BlockGenerator(newTransaction, pm,socket,bc,km); bg.start(); }
			 */
			while ((newBlock = (Block) ois.readObject()) != null) {
				// System.out.println(socket.getInetAddress()+" : "+socket.getPort());
				System.out.println("[RPC]Get Transaction: "
						+ newBlock.getTransaction());
				bg = new BlockGenerator(newBlock, pm, socket, bc, km,bnm);
				bg.start();
			}

		} catch (Exception e) {
			deleteRPCFromDB();
			System.out.println("[RPC]Peer " + socket.getInetAddress()
					+ " disconnected.");
		}
	}

	/**
	 * * Doesn't actually 'read data' as that's done asynchronously in the
	 * threadded run function. However, readData is an easy way to think about
	 * it--as receivedData acts as a buffer, holding received data until the
	 * daemon is ready to handle it. Generally, the size of receivedData will be
	 * small. However, in some instances (like when downloading many blocks), it
	 * can grow quickly.
	 * 
	...
	public ArrayList<Transaction> readData() {
		...
		System.out
				.println("We have " + receivedTransaction.size() + " pieces!");
		// Don't want to mess with the ArrayList while run() is modifying it.
		ArrayList<Transaction> inputBuffer = new ArrayList<Transaction>(
				receivedTransaction);
		if (inputBuffer == null) {
