...
{ 
    ...
    public RPC(PeerNetworkManager pm,Blockchain bc,KeyManager km) 
    { 
        this.listenPort = 8016; 
        this.rpcThreads = new ArrayList<RPCThread>();
        this.pm = pm;
    ...
    { 
        ...
        { 
        	if(!isSelf(peer)) {
            	...
            	rpcThreads.add(new RPCThread(socket,pm,bc,km)); 
...
//            	if(!isSelf(cSocket.get(cSocket.size()-1).getInetAddress())) {
            		rpcThreads.add(new RPCThread(listenSocket.accept(),pm,bc,km)); 
            		rpcThreads.get(rpcThreads.size() - 1).start();
 //               }
            }  
            listenSocket.close();
