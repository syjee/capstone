...
//					throw new NoSuchAlgorithmException();
			//	else request.getTransaction().setTransaction(str);
				...
				if(!request.getMerkleRoot().equals(mt.getRoot())){
					...
					throw new Exception("[MerkleTree]A MerkleRoot is not matched!");
				}else System.out.println("[MerkleTree]A MerkleRoot is matched!");
				
				this.merkleRoot = mt.getRoot();
				
				if(bm.getHeight() != 0){
					newBlock = new Block(request.getIndex(), request.getArrayTransaction(),bm.getRecentBlock().getCurrentHash(),merkleRoot);
					System.out.println("height = "+bm.getHeight());
				}
				else if(bm.getHeight() == 0){
					newBlock = new Block(request.getIndex(), request.getArrayTransaction(),merkleRoot);
