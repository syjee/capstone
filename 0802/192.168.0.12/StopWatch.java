...
public class StopWatch {
	...
	static Long prvTime;
	static Long avgDelay = (long) 1900;
	...
	public TimeReception tr = new TimeReception();
	public Long msec;
	...
	public void newStopWatch(int onOff) {
		...
		{
			cal.setTimeInMillis(Long.parseLong(time) + System.currentTimeMillis() - prvTime );
			ts.StringTimeSync(form.format(cal.getTime()));
			System.out.println("[TIME]Timer OFF!!!");
			
			tr.timereceive();
			db.addtoFile(db.getHistoryFile(), "TimeStamp : "+cal.getTime() +", Offset : " + tr.offsetValue+"(ms)");
